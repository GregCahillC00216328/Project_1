...
void Game::run()
{
	...
	while (isRunning){
		...
		while (window.pollEvent(event))
		{
			...
			{
				// Set Model Rotation
				model[3] = translate(model[3], glm::vec3(-0.2, 0.0, 0.0));
			}
...
void Game::initialize()
{
	...
	for (int i = 0; i < 3; i++)
	{
		glGenBuffers(i, &vbo);		// Generate Vertex Buffer
	...
	model[0] = translate(model[0], glm::vec3(-6.0f, 2.0f, -5.0f));
	model[1] = mat4(
		1.0f					// Identity Matrix
	);
	model[1] = translate(model[1], glm::vec3(0.0f, 2.0f, -5.0f));
	model[2] = mat4(
		1.0f					// Identity Matrix
	);
	model[2] = translate(model[2], glm::vec3(6.0f, 2.0f, -5.0f));
	model[3] = mat4(
		1.0f					// Identity Matrix
	);
	model[3] = translate(model[3], glm::vec3(0.0f, -2.0f, 5.0f));
...
void Game::render()
{
	...
	glUniformMatrix4fv(mvpID[0], 1, GL_FALSE, &mvp[0][0][0]);
	
	//glUniformMatrix4fv(mvpID[2], 1, GL_FALSE, &mvp3[0][0]);
	...
	glUniformMatrix4fv(mvpID[1], 1, GL_FALSE, &mvp[1][0][0]);
	glDrawElements(GL_TRIANGLES, 3 * INDICES2, GL_UNSIGNED_INT, NULL);
	glBufferSubData(GL_ARRAY_BUFFER, 0 * VERTICES3 * sizeof(GLfloat), 3 * VERTICES3 * sizeof(GLfloat), vertices3);
	glBufferSubData(GL_ARRAY_BUFFER, 3 * VERTICES3 * sizeof(GLfloat), 4 * COLORS3 * sizeof(GLfloat), colors3);
	glBufferSubData(GL_ARRAY_BUFFER, ((3 * VERTICES3) + (4 * COLORS3)) * sizeof(GLfloat), 2 * UVS3 * sizeof(GLfloat), uvs);
	glUniformMatrix4fv(mvpID[2], 1, GL_FALSE, &mvp[2][0][0]);
	glDrawElements(GL_TRIANGLES, 3 * INDICES3, GL_UNSIGNED_INT, NULL);
	glBufferSubData(GL_ARRAY_BUFFER, 0 * VERTICES4 * sizeof(GLfloat), 3 * VERTICES4 * sizeof(GLfloat), vertices4);
	glBufferSubData(GL_ARRAY_BUFFER, 3 * VERTICES4 * sizeof(GLfloat), 4 * COLORS4 * sizeof(GLfloat), colors4);
	glBufferSubData(GL_ARRAY_BUFFER, ((3 * VERTICES4) + (4 * COLORS4)) * sizeof(GLfloat), 2 * UVS4 * sizeof(GLfloat), uvs);
	glUniformMatrix4fv(mvpID[2], 1, GL_FALSE, &mvp[3][0][0]);
	glDrawElements(GL_TRIANGLES, 3 * INDICES3, GL_UNSIGNED_INT, NULL);
	window.display();
